...
- **2026‑05‑27** — §6 gains a Banned phrases catalogue + Anecdote fidelity rule. New §7a "How to write a piece (the recipe)" with the kernel-plus-six-sections shape, required components, length window, tag pattern, bilingual rules. §9 gains a Diagrams subsection lifting PR P4's role-class pattern. ADR 0011 documents the piece shape with the four P3 pieces as worked examples. Four advisory unit tests (em dash, banned phrases, margin-note anchors, piece shape) enforce the rules. Why: the four P3 pieces independently converged on the same shape — codifying it lets future pieces match without re-deriving, and the tests catch silent drift.
...
Enforced via `tests/unit/piece-banned-phrases.test.ts`. Advisory for the first month after PR V ships — the catalogue might still be tuning. Promote to fail-on-CI once the false-positive rate is acceptable.
