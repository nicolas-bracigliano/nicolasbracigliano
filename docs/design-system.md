...
- **Single source of truth.** `buildCmdkIndex(locale)` (`src/lib/cmdk-index.ts`) reads the same `notes` / `works` / `pieces` content collections the routes render, plus the static route list. There is no separate index to maintain — it's a prerendered per-locale endpoint (`/cmdk/<locale>.json`, from `src/pages/cmdk/[lang].json.ts`) that `src/scripts/cmdk.ts` fetches once on first open and caches; matching is client-side. Keeping the index out of the page HTML holds every page under the ~50 KB weight budget, and there's no wasm. It works under `astro dev` too — the client requests the `trailingSlash`-correct URL via `import.meta.env.DEV`.
