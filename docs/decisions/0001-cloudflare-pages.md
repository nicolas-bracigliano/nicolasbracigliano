...
- `wrangler.toml` gained `run_worker_first = true` so the Worker fires for every request (PR #54). Without it, the `/` redirect was dead code: Workers Static Assets' default `not_found_handling = "404-page"` bypasses the Worker when no asset matches. **Amended 2026-07-30:** narrowed from `true` to the explicit list `["/", "/.well-known/security.txt"]` — the same two paths `src/worker.ts` branches on. `true` was routing every asset request through the Worker only to fall through to `env.ASSETS.fetch()`, paying an invocation and a hop for ~100% of traffic. The fix for the original bug is unchanged; only its blast radius narrowed.
